...
local function owned(state, id)
  return item_queries.total_owned(state, id) > 0
end

local function needs_loadout(state, loadout)
  return not loadouts.matches_carried(state, loadout)
end

local function needs_minimum_loadout(state, loadout)
  return #loadouts.missing_carried(state, loadout) > 0
end
...
local function resolve_investigation(state, world)
  if state.varp == 1 then
    return geometry.in_zone(world, config.zones.hudon_island) and "talk_hudon" or "reach_hudon"
  end
  if item_queries.carried_in(state, config.items.book) > 0 then return "read_book" end
  if geometry.in_zone(world, config.zones.tourist_upstairs) then return "obtain_book" end
  if geometry.in_zone(world, config.zones.ledge) then return "leave_ledge" end
  if geometry.in_zone(world, config.zones.dead_tree_island) then return "descend_tree" end
  if geometry.in_zone(world, config.zones.hudon_island) then return "cross_to_tree" end
  return "reach_tourist_stairs"
end

local function resolve_final(state, world)
  if geometry.in_zone(world, config.zones.chalice_room) then return "finish_quest" end
  if geometry.in_zone(world, config.zones.pillar_room) then
    return item_queries.carried_in(state, config.items.amulet) == 0 and "amulet_missing_in_pillar_room" or
      item_queries.quantity(state.equipment, config.items.amulet) > 0 and "remove_amulet" or
      "charge_pillars"
  end
  if geometry.in_zone(world, config.zones.falls) then
    if item_queries.carried_in(state, config.items.baxtorian_key) > 0 then return "open_inner_door" end
  ...
  if geometry.in_zone(world, config.zones.ledge) then
    return item_queries.quantity(state.equipment, config.items.amulet) > 0 and
      "enter_falls" or "equip_amulet"
  end
  if geometry.in_zone(world, config.zones.dead_tree_island) then return "descend_tree_final" end
  if geometry.in_zone(world, config.zones.hudon_island) then return "cross_to_tree_final" end
...
local function resolve_items(state, world)
  ...
  if has_amulet and has_urn then
    if geometry.in_zone(world, config.zones.gnome_basement) then return "leave_gnome_dungeon" end
    if geometry.in_zone(world, config.zones.glarial_tomb) then return "leave_glarial_tomb" end
    return resolve_final(state, world)
  end
  if geometry.in_zone(world, config.zones.glarial_tomb) then
    return item_queries.carried_in(state, config.items.amulet) == 0 and "obtain_amulet" or "obtain_urn"
  end
  if item_queries.carried_in(state, config.items.pebble) > 0 then
    local loadout = required_tomb_loadout(state)
    if geometry.in_zone(world, config.zones.gnome_basement) then return "leave_gnome_dungeon" end
    if geometry.distance(world, config.points.tombstone) <= 12 then
  ...
  if geometry.in_zone(world, config.zones.golrie_room) then return "obtain_pebble" end
  if geometry.in_zone(world, config.zones.gnome_basement) then
    return item_queries.carried_in(state, config.items.golrie_key) > 0 and
      "open_golrie_gate" or "obtain_golrie_key"
  end
  if geometry.in_zone(world, config.zones.tourist_upstairs) then return "leave_tourist_house" end
