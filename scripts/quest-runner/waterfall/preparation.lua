...
local function execute(phase, restock)
  if phase == "bank_unknown" then
    ...
    local final_items_owned = item_queries.total_owned(state, config.items.amulet) > 0 and
      item_queries.total_owned(state, config.items.urn) > 0
  ...
  if phase == "tomb_loadout_required" then
    local state = state_api.read("waterfall")
    if item_queries.total_owned(state, config.items.amulet) > 0 then
  ...
  if phase == "final_loadout_required" then
    local state = state_api.read("waterfall")
    if item_queries.total_owned(state, config.items.baxtorian_key) > 0 then
