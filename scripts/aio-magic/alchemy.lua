local behaviors = gc.require("shared_behaviors")
local config = gc.require("config")
local item_queries = gc.require("shared_items")
local preparation = gc.require("preparation")
local progress = gc.require("progress")
local ui = gc.require("shared_ui")

local method = config.low_alchemy

local function remaining_casts(current_xp, required_xp)
  if current_xp >= required_xp then return 0 end
  return math.ceil((required_xp - current_xp) / method.xp_per_cast)
end

local function supply_plan(casts)
  return {
    {
      id = method.staff_id,
      name = "Staff of fire",
      quantity = 1,
      maximum_unit_price = 2000,
    },
    {
      id = method.nature_rune.id,
      name = method.nature_rune.name,
      quantity = casts,
      maximum_unit_price = method.nature_rune.maximum_unit_price,
    },
    {
      id = method.item.id,
      name = method.item.name,
      quantity = casts,
      maximum_unit_price = method.item.maximum_unit_price,
    },
  }
end

local function wait_for_cast(before_xp, before_items, ticks)
  for _ = 1, ticks do
    gc.await { event = "game.tick" }
    local magic = gc.read("skills").magic
    local items = item_queries.inventory_quantity(method.item.id)
    if magic.xp > before_xp and items < before_items then return magic end
  end
  return nil
end

local function terminal(status, target, start_xp, casts, receipt)
  local magic = gc.read("skills").magic
  progress.show(target, status == "complete" and "Complete" or "Stopped")
  ui.park_mouse()
  local result = {
    status = status,
    target_level = target,
    start_xp = start_xp,
    final_xp = magic.xp,
    gained_xp = magic.xp - start_xp,
    final_level = magic.level,
    method = "low_alchemy",
    completed_casts = casts,
  }
  if receipt then result.receipt = receipt end
  return result
end

local function run(options)
  local target = options.target
  local required_xp = options.required_xp
  local start_xp = options.start_xp
  local restock = options.restock
  local magic = gc.read("skills").magic
  if magic.level < method.minimum_level then
    return {
      status = "low_alchemy_magic_required",
      magic_level = magic.level,
      required_magic_level = method.minimum_level,
    }
  end

  local configured, behavior_failure = behaviors.configure {
    auto_retaliate = true,
    emergency_consumables = false,
    emergency_escape = false,
    combat_prayer = false,
  }
  if not configured then return behavior_failure end
  gc.await { action = { type = "safety.clear" }, policy = { breaks = false, cursor_release = "none", fidget = "none" } }

  local at_ge, ge_error = preparation.ensure_at_ge(target)
  if not at_ge then return ge_error end
  gc.phase("magic.low_alchemy.bank", { activity = "skilling" })

  magic = gc.read("skills").magic
  local casts = remaining_casts(magic.xp, required_xp)
  local supplied, supply_error = preparation.ensure_supplies(
    supply_plan(casts), restock, target, "items")
  if not supplied then return supply_error end

  progress.show(target, "Loading alchemy supplies")
  local loadout = gc.await {
    action = {
      type = "bank.loadout",
      items = {
        { id = method.staff_id, quantity = 1 },
        { id = method.nature_rune.id, quantity = casts },
        { id = method.item.id, quantity = casts },
      },
      minimum_free_slots = 25,
      close = true,
    },
    timeout = { game_ticks = 240 },
  }
  if loadout.status ~= "complete" then
    return {
      status = "low_alchemy_loadout_failed",
      receipt = loadout,
    }
  end
  local equipped, equip_error = preparation.equip_staff(
    method.staff_id, target, method.staff_name)
  if not equipped then
    return { status = "low_alchemy_staff_failed", receipt = equip_error }
  end

  local failures = 0
  local completed_casts = 0
  while true do
    magic = gc.read("skills").magic
    if magic.xp >= required_xp or magic.level >= target then break end
    if gc.next_action() == "stop_after_cast" then
      return terminal("stopped", target, start_xp, completed_casts)
    end

    local before_xp = magic.xp
    local before_items = item_queries.inventory_quantity(method.item.id)
    if before_items < 1 or item_queries.inventory_quantity(method.nature_rune.id) < 1 then
      return {
        status = "low_alchemy_supplies_exhausted",
        level = magic.level,
        xp = magic.xp,
      }
    end

    progress.show(target, "Casting Low Alchemy")
    local cast = gc.await {
      action = {
        type = "spell.cast_on_item",
        spell = method.spell,
        item_id = method.item.id,
      },
      timeout = { game_ticks = 20 },
    }
    if cast.status ~= "dispatched" then
      failures = failures + 1
      gc.log("warn", "low-alchemy-cast-retry", cast)
    elseif wait_for_cast(before_xp, before_items, 8) then
      failures = 0
      completed_casts = completed_casts + 1
    else
      failures = failures + 1
      gc.log("warn", "low-alchemy-xp-unchanged", {
        xp = before_xp,
        items = before_items,
        receipt = cast,
        messages = gc.read("messages", { limit = 8 }),
      })
    end
    if failures >= 3 then
      return {
        status = "low_alchemy_cast_unconfirmed",
        level = magic.level,
        xp = magic.xp,
        failures = failures,
      }
    end
  end

  gc.phase("magic.target_reached")
  local result = terminal("complete", target, start_xp, completed_casts)
  gc.log("info", "magic-complete", result)
  return result
end

return {
  remaining_casts = remaining_casts,
  supply_plan = supply_plan,
  run = run,
}
