...
final class GrandTree extends QuestWorkflow
{
	...
	GrandTree() { super("the_grand_tree",150); }
	...
	@Override void execute(String phase)
	{
		...
		switch (phase)
		{
			...
			case "start": travel.king(false); talk(GnomeTravel.KING,null,() -> varp() >= 10,false,"You seem worried, what's up?","Yes.","I'd be happy to help!"); break;
			case "hazelmere": travel.hazelmere(); talk(new int[]{1422,13610},null,() -> varp() >= 20,false); break;
			case "translation": travel.king(false); talk(GnomeTravel.KING,null,() -> varp() >= 30,false,"I think so!","A man came to me with the King's seal.","I gave the man Daconia rocks.","And Daconia rocks will kill the tree!","None of the above."); break;
			case "glough": travel.glough(); talk(GnomeTravel.GLOUGH,null,() -> varp() >= 40,false); break;
			case "king_after_glough": travel.king(false); talk(GnomeTravel.KING,null,() -> varp() >= 50,false); break;
			case "charlie": travel.top(); talk(GnomeTravel.CHARLIE,null,() -> varp() >= 60,false); break;
			...
			case "charlie_cell": talk(GnomeTravel.CHARLIE,null,() -> varp() >= 80,false); break;
			...
			case "foreman": talk(new int[]{1429},new Tile(3000,3044),() -> varp() >= 90 || Inventory.contains(787),false,"Sadly his wife is no longer with us!","He loves worm holes.","Anita."); break;
			case "lumber_order": travel.king(true); travel.top(); talk(GnomeTravel.CHARLIE,null,() -> varp() >= 100,false); break;
			...
			case "return_plans": travel.king(false); talk(GnomeTravel.KING,null,() -> varp() >= 120,false); break;
	...
	private void twigs()
	{
		...
		Automation.intent("grand_tree.place_tuzo_twigs", () ->
		{
			...
			await(() -> varp() >= 130,30,"Tuzo puzzle completion was not observed");			return null;
		});
