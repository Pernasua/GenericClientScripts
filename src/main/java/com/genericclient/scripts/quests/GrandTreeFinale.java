...
final class GrandTreeFinale
{
	...
	void execute(String phase)
	{
		switch (phase)
		{
			case "prepare_combat": quest.prepare(List.of(new Supply(1387,"Staff of fire",1,5000),new Supply(558,"Mind rune",300,20),
				new Supply(556,"Air rune",600,20),new Supply(379,"Lobster",6,500),Jewellery.DUELING_RING)); break;
	...
	private void enterCave(boolean afterFight)
	{
		...
		WorkflowScript.require(hatch != null,"Glough's trapdoor was not observed");
		String action = hatch.hasAction("Climb-down") ? "Climb-down" : "Open";
		WorkflowScript.require(hatch.interact(action),"Glough's trapdoor interaction failed");
		WorkflowScript.awaitTicks(this::inCave,40,"Demon tunnel entry was not observed");
	}
	private void fight()
	{
		QuestCombat.configure(1387,Normal.FIRE_STRIKE);
		Safety.guard(18,Safety.lobster(),true,null);
		enterCave(false);
		for (int tick = 0; tick < 80 && QuestWorkflow.npc(1432) == null; tick++) { Conversations.finish(); Sleep.sleepTicks(1); }
		WorkflowScript.require(QuestWorkflow.npc(1432) != null,"Black demon did not spawn");
		...
		WorkflowScript.require(demon != null && demon.distance() >= 4 && QuestCombat.lineOfSight(demon),"Black demon safespot was not established");
		QuestCombat.monitor(new int[]{1432},() -> quest.stage() >= 140,900,
			target -> WorkflowScript.require(QuestWorkflow.tile().equals(safe),"Black demon safespot was lost"));
	}
	private void reachCaveKing()
	{
		...
		WorkflowScript.require(QuestWorkflow.npc(GnomeTravel.KING) != null,"Cave king was not observed");
	}
	private void findRock()
	{
		...
		for (int scan = 0; scan < 3; scan++)
		{
			...
			for (GameObject root : roots)
			{
				...
				WorkflowScript.require(root.interact("Search"),"Daconia root search failed");
