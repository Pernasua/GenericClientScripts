...
final class WaterfallRitual
{
	...
	void execute(String phase)
	{
		switch (phase)
		{
			...
			case "inner_door":
				...
				if (door != null) WorkflowScript.require(door.interact("Open"),"Inner waterfall door did not open");
				quest.walk(new Tile(2566,9903),0,true); break;
			case "remove_amulet":
				WorkflowScript.require(Equipment.get(item -> item.getId() == 295).interact("Remove"),"Glarial's amulet was not removed");
				WorkflowScript.awaitTicks(() -> Inventory.contains(295),12,"Glarial's amulet was not returned to inventory"); break;
	...
	private void chargePillars()
	{
		...
		WorkflowScript.require(pillars.size() == 6,"Waterfall requires six observed pillars");
		for (Tile point : pillars)
		{
			...
			for (int rune : new int[]{556,555,557})
			{
				...
				WorkflowScript.require(before > 0,"Ritual runes are missing");
				long tick = ScriptScope.current().tick();
				GameObject pillar = QuestWorkflow.object(2005,point);
				WorkflowScript.require(pillar != null && Inventory.get(rune).useOn(pillar),"Ritual rune placement failed");
				WorkflowScript.awaitTicks(() -> Inventory.count(rune) < before || QuestWorkflow.message(tick,"already"),8,"Ritual rune placement was not verified");
			}
		}
	...
	private void finish()
	{
		WorkflowScript.require(!Inventory.contains(297),"The urn is empty");
		Conversations.finish();
		GameObject chalice = GameObjects.closest(2014);
		WorkflowScript.require(chalice != null,"Chalice was not observed");
		quest.walk(chalice.getTile(),3,true);
		WorkflowScript.require(Inventory.get(296).useOn(chalice),"Urn could not be placed in the chalice");
