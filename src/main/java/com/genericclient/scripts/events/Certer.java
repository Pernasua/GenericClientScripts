...
public final class Certer extends WorkflowScript
{
	...
	@Override protected Object runWorkflow()
	{
		...
		{
			EventSupport.enter("certer.answer_question",() -> Interfaces.widget(ITEM) != null);
			int model = Interfaces.widget(ITEM).getModelId();
			String expected = ANSWERS.get(model);
			require(expected != null,"Unrecognized Certer model: " + model);
			awaitTicks(() -> Interfaces.widget(12058625) != null && Interfaces.widget(12058626) != null && Interfaces.widget(12058627) != null,
				10,"Certer labels did not appear");
			int answer = -1;
			for (int index = 0; index < 3; index++)
			{
				if (normalize(Interfaces.widget(12058625+index).getText()).equals(normalize(expected)))
				{
			...
			Interfaces.click(12058632+answer);
